...
%-----------------------------------------------------------
%   Significance of temporal discretization
%-----------------------------------------------------------

dt = [0.001 0.005 0.01 0.05 0.1 0.2 0.5 1];
N = 21;
terminal_velocity = zeros(length(dt),1);

for kk = 1:length(dt)
    terminal_velocity(kk) = implicit_N_nodes(dt(kk),N);
end
...
%-----------------------------------------------------------
%   Significance of spatial discretization
%-----------------------------------------------------------

dt = 0.01;
N = [3,5,11,21,51,101];
terminal_velocity = zeros(length(N),1);

for kk = 1:length(N)
    terminal_velocity(kk) = implicit_N_nodes(dt,N(kk));
end
...
%-----------------------------------------------------------
%   Velocity of the middle node
%-----------------------------------------------------------

middle_velocity_y = u_solution(:,middle_index*2);

terminal_velocity = middle_velocity_y(end);
